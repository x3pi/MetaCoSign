...
func (p *RpcReverseProxy) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	case "mtn_sendRawTransactionWithDeviceKey":
		rawTx := gjson.GetBytes(body, "params.0")
		if !rawTx.Exists() {
			resp := utils.MakeInvalidParamError(id, "Invalid params for sendRawTransactionWithDeviceKey")
			utils.WriteJSON(w, resp)
			return
		}
		isPrevent, hash, err := handlers.ProcessSendRawTransactionWithDeviceKey(p.AppCtx, rawTx.String(), id)
		if isPrevent {
			if err != nil {
				utils.WriteJSON(w, utils.MakeInternalError(id, err.Error()))
			} else {
				utils.WriteJSON(w, rpc_client.JSONRPCResponse{Jsonrpc: "2.0", Id: id, Result: hash})
			}
			return
		}
		// Not an upload chunk, forward to upstream
		r.Body = io.NopCloser(bytes.NewReader(body))
		p.ReverseProxy.ServeHTTP(w, r)
		return
	case "net_version":
