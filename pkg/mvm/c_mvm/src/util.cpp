...
namespace mvm
{
  ...
  // Hàm mã hóa RLP
  std::vector<uint8_t> rlp_encode(const std::vector<uint8_t> &sender, uint64_t nonce)
  {
    ...
    else
    {
      ...
      else
      {
        ...
        std::vector<uint8_t> length_encoded = {static_cast<uint8_t>(0x80 + nonce_bytes.size())};
        nonce_bytes.insert(nonce_bytes.begin(), length_encoded.begin(), length_encoded.end());
      }
    }

    // Tính toán tổng độ dài của RLP list
    size_t total_length = sender.size() + nonce_bytes.size() + 1; // +1 cho sender prefix
    ...
    else
    {
      ...
      while (temp_length > 0)
      {
        length_bytes.insert(length_bytes.begin(), static_cast<uint8_t>(temp_length & 0xFF));
        temp_length >>= 8;
      }
  ...
  {
    std::stringstream ss;
    ss << std::hex << std::setfill('0') << std::setw(40); // 40 characters for 20 bytes (20 * 2 hex digits)
  ...
  Address generate_contract_address_with_nonce(const Address &sender, uint256_t sender_last_hash, size_t nonce)
  {
    ...
    keccak_256(
        (unsigned char *)byte_data,
        static_cast<unsigned int>(byte_data_size), // Sử dụng kích thước chính xác của byte_data
        buffer);
  ...
  Address generate_contract_address_2(const Address &sender, uint256_t salt, vector<uint8_t> init_code)
  {
    uint8_t b_sender[32];
    mvm::to_big_endian(sender, b_sender);
...
std::vector<uint8_t> encode_abi_bytes(const std::vector<uint8_t> &data)
{
    ...
    // Nếu data_actual_size là bội số của 32, không cần padding (padding_needed = 0).
    // Ngược lại, padding_needed = 32 - (data_actual_size % 32).
    ...
    // Khởi tạo vector 'result' với kích thước cuối cùng, các phần tử được khởi tạo bằng 0.
    std::vector<uint8_t> result(total_size, 0);

    // 1. Offset (32 byte đầu tiên)
    // Vector 'result' đã được điền sẵn bằng 0, chỉ cần đặt byte cuối của phần offset.
    if (total_size >= 32) { // Đảm bảo vector đủ lớn (luôn đúng nếu total_size tính đúng)
      result[31] = 0x20;
    }
    ...
    if (total_size >= 64) { // Đảm bảo vector đủ lớn
        ...
        // Sử dụng một buffer tạm thời nếu to_big_endian yêu cầu con trỏ không trỏ vào result đang được xây dựng
        // hoặc nếu bạn muốn đảm bảo to_big_endian không bị ảnh hưởng bởi các giá trị khác trong result.
        // Tuy nhiên, ở đây ta có thể ghi trực tiếp vào result.begin() + 32 nếu to_big_endian an toàn.
        to_big_endian(length_val, result.data() + 32);
    }

    // 3. Data bytes (bắt đầu từ chỉ số 64)
    if (data_actual_size > 0 && total_size >= 64 + data_actual_size) { // Đảm bảo có dữ liệu và đủ chỗ
        std::copy(data.begin(), data.end(), result.begin() + 64);
    }

    // 4. Padding (các byte cuối cùng)
    // Đã được xử lý vì vector 'result' được khởi tạo với 'total_size' và điền sẵn bằng 0.
    // Không cần hành động thêm nếu các byte padding phải là 0.
