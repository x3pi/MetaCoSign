...
namespace mvm
{
  ...
  public:
        ...
        bool readOnly)
    {
      ...
      auto eh = [&](const Exception &ex_)
      {
        ...
        {
          result.output = mvm::to_bytes(ex_.what());
        }
  ...
  private:
        ...
        vector<uint8_t> &dst, const vector<uint8_t> &src, const uint8_t pad)
    {
      ...
      gas_tracker.add_gas_used(
          getCopyOperationGasCost(size, ctxt->last_mem_gas_cost, old_mem_word_size, new_mem_word_size));
    }
    ...
    vector<uint8_t> copy_from_mem(const uint64_t offset, const uint64_t size)
    {
      ...
      uint64_t new_mem_word_size = ctxt->get_used_mem();
      gas_tracker.add_gas_used(
          getCopyOperationGasCost(get_word_size(size), ctxt->last_mem_gas_cost, old_mem_word_size, new_mem_word_size));
    ...
    void saveDebugInfo(const Transaction &tx, uint8_t op, const Context *ctxt)
    {
      ...
      {
        std::cerr << "Lỗi: Không thể tạo thư mục " << directory << " (" << ec.message() << ")" << std::endl;
        return;
      }
      ...
      outFile << "Op code: 0x" << std::hex << static_cast<unsigned int>(op) << std::endl;
      outFile << "Op string: " << std::hex << mvm::opcodeToString(op) << std::endl;
      ...
      {
        outFile << std::hex << std::setw(2) << std::setfill('0') << static_cast<int>(ctxt->mem[i]);
        if ((i + 1) % 32 == 0)
        {
          outFile << std::endl;
        }
    ...
    void mcopy()
    {
      ...
      {
        // Copy từ cuối lên đầu để tránh ghi đè dữ liệu nguồn trong trường hợp chồng chéo
    ...
    void mload()
    {
      ...
      uint64_t new_mem_word_size = ctxt->get_used_mem();
      gas_tracker.add_gas_used(
          getMemExpansionGasCost(ctxt->last_mem_gas_cost, old_mem_word_size, new_mem_word_size));
    }
    ...
    void number()
    {
      ctxt->s.push(gs.get_block_context().number);
    }
    ...
    void coinbase()
    {
      ctxt->s.push(gs.get_block_context().coinbase);
    }

    void timestamp()
    {
      ctxt->s.push(gs.get_block_context().time);
    }

    void prevrandao()
    {
      ctxt->s.push(gs.get_block_context().prevrandao);
    }
    ...
    void gaslimit()
    {
      ctxt->s.push(gs.get_block_context().gas_limit);
    }
    ...
    void sha3()
    {
      ...
      uint64_t new_mem_word_size = ctxt->get_used_mem();
      gas_tracker.add_gas_used(
          getSha3GasCost(get_word_size(size), ctxt->last_mem_gas_cost, old_mem_word_size, new_mem_word_size));
    }
    ...
    void create()
    {
      ...
      Address newAddress = generate_address(ctxt->acc.get_address(), ctxt->acc.get_nonce());
      ...
      auto rh = [newAcc, parentContext, this](vector<uint8_t> output)
      {
        ...
        gas_tracker.add_gas_used(
            getCodeDepositCost(output.size()));
      };
    ...
    void call()
    {
      ...
      const auto value = (op == DELEGATECALL || op == STATICCALL) ? 0 : ctxt->s.pop();
      ...
      bool is_precompile =
          ...
          (addr == EXTRACT_JSON_FIELD_EXTENSION) ||
          (addr == BLST) ||
          (addr == MATH_EXTENSTON_ADDRESS) ||
          (addr == SIMPLE_DATABASE_ADDRESS) ||
          (addr == FULL_DATABASE_ADDRESS);
      ...
      if (is_precompile)
      {
        ...
        try
        {
          ...
          else if (addr == 400)
          {
            ...
            for (int i = 0; i < 32; i++)
            {
              precompile_output[i] = static_cast<uint8_t>((tx.tx_hash >> (8 * (31 - i))) & 0xFF);
            }
          }
          ...
          else if (addr == SIMPLE_DATABASE_ADDRESS)
          {
            ...
            {
              throw Exception(ET::ErrWriteProtection, "SimpleDatabase write protection");
            }
            precompile_output = extension.SimpleDatabase(input, ctxt->as.acc.get_address());
          }
          else if (addr == FULL_DATABASE_ADDRESS)
          {
            precompile_output = extension.FullDatabase(input, ctxt->as.acc.get_address(), false, gs.get_block_context().number);
          }
          else
          {
            throw Exception(ET::ErrInvalidCode, "Precompiled contract not implemented.");
          }
        }
          ...
          [offOut, sizeOut, parentContext, this](const vector<uint8_t> &output)
      {
        ...
        copy_mem_raw(offOut, 0, sizeOut, parentContext->mem, parentContext->returnData);
      ...
      auto he =
          [this, parentContext, offOut, sizeOut](const Exception &e)
      {
    ...
    {
      ctxt->s.push(ctxt->returnData.size());
    }
    ...
    void opSAR()
    {
      ...
      if (shift > 256)
      {
        ctxt->s.push(sign < 0 ? to_uint256("0xffffffffffffffffffffffffffffffffffffffff") : uint256_t(0));
        return;
      }
    ...
    void opBaseFee()
    { 
      ctxt->s.push(gs.get_block_context().base_fee);
    }
    ...
    void opCreate2()
    {
      ...
      gas_tracker.add_gas_used(
          getCreate2DataSizeCost(get_word_size(size)));

      Address newAddress = generate_contract_address_2(ctxt->acc.get_address(), salt, input);
  ...
  Processor::Processor(
      ...
      LogHandler &log_handler,
      Extension &extension,
      NativeLogger &native_logger) : gs(gs),
                                     log_handler(log_handler),
                                     extension(extension),
                                     native_logger(native_logger)
  {
