...
namespace mvm
{
  ...
  inline double uint256_to_double(const intx::uint256 &value)
  {
    ...
    // vì double có thể biểu diễn chính xác tất cả các giá trị uint64_t lên đến 2^53.
    // Chuyển đổi uint64_t -> double thường nhanh hơn và tránh cấp phát chuỗi.
    const intx::uint256 max_uint64 = std::numeric_limits<uint64_t>::max();
    if (value <= max_uint64)
    {
      uint64_t val64 = static_cast<uint64_t>(value);
      // Cảnh báo nếu giá trị vẫn lớn hơn giới hạn biểu diễn nguyên chính xác của double
      if (val64 > (1ULL << std::numeric_limits<double>::digits))
      { // digits thường là 53
    ...
    try
    {
      ...
      std::cerr << "Warning: Converting large uint256_t value to double via string."
    ...
    catch (const std::out_of_range &oor)
    {
      ...
      std::cerr << "Error converting uint256_t to double: Value out of range for double representation. "
                << oor.what() << std::endl;
    ...
    catch (const std::invalid_argument &ia)
    {
      ...
      std::cerr << "Error converting uint256_t to double: Invalid argument for std::stod. "
                << ia.what() << std::endl;
      return std::numeric_limits<double>::quiet_NaN(); // Trả về Not-a-Number
    }
    catch (const std::exception &e)
    {
      ...
      std::cerr << "Error converting uint256_t to double: " << e.what() << std::endl;
      return std::numeric_limits<double>::quiet_NaN();
    }
  }
  ...
  Address generate_contract_address_2(const Address &sender, uint256_t salt, std::vector<uint8_t> init_code);
  uint64_t to_uint64(const std::string &s);
  void print_hex(uint8_t *s, size_t len);
  int getSignUint256(uint256_t v);
  inline uint64_t get_word_size(uint64_t size)
  {
    return (size + 32u - 1) / 32u;
  };
  uint256_t getPaddedAddressSelector(const std::string &functionSignature);
  int private_to_public(const unsigned char *private_key, unsigned char *public_key);
