...
func processDecodedLogs(result *MVMExecuteResult) {
	...
	for addr, serializedLogData := range result.MapFullDbLogs {
		...
		if err != nil {
			fmt.Printf("  Error deserializing log data for address %s: %v\n", addr, err)
			continue
		}

		// In ra cấu trúc đã giải mã (dùng JSON để dễ đọc)
		...
		// fmt.Printf("  Number of Schema Logs: %d\n", len(decodedLog.SchemaLogs))
		// if len(decodedLog.XapianDocLogs) > 0 {
		// 	fmt.Printf("  First Xapian Op: %v\n", decodedLog.XapianDocLogs[0].Op)
		// }
	}
