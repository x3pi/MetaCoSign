...
namespace mvm
{
    ...
    {
        // === ĐỌC ĐƯỜNG DẪN CƠ SỞ TỪ BIẾN MÔI TRƯỜNG ===
        std::string xapianBasePath = ""; // Giá trị mặc định
        const char *path_env = std::getenv("XAPIAN_BASE_PATH");
        if (path_env)
        {
            xapianBasePath = path_env;
            // Bạn có thể bỏ dòng cout này trong code production
            // std::cout << "[createFullPath] Using XAPIAN_BASE_PATH from environment: " << xapianBasePath << std::endl;
        }
        else
        {
            // Bạn có thể bỏ dòng cerr này nếu không muốn cảnh báo
            std::cerr << "[createFullPath Warning] Environment variable XAPIAN_BASE_PATH not set. Using default: " << xapianBasePath << std::endl;
            // Hoặc throw lỗi nếu bắt buộc phải có:
            // throw std::runtime_error("XAPIAN_BASE_PATH environment variable is required but not set.");
        }
        // ==============================================
        ...
        // Tạo đường dẫn sử dụng đường dẫn cơ sở đã đọc được
        std::filesystem::path fullPath = std::filesystem::path(xapianBasePath) / // <-- Sử dụng biến đã đọc
                                         intx::to_string(address) /
                                         hashedDbname;
    ...
    std::vector<uint8_t> evm_encode_mpfr(const mpfr_t &value)
    {
        ...
        size_t padded_len = std::ceil(static_cast<double>(len) / WORD_SIZE) * WORD_SIZE;
        ...
        std::copy(length_bytes.begin(), length_bytes.end(), encoded.begin() + WORD_SIZE);

        // Thêm nội dung chuỗi vào bộ đệm đã padding
        std::copy(str_value_str.begin(), str_value_str.end(), encoded.begin() + 2 * WORD_SIZE);
    ...
    void hexToSignedInt(mpfr_t result, const std::vector<uint8_t> &bytes)
    {
        ...
        { // MSB = 1 → số âm
            ...
            mpfr_ui_pow_ui(two_power_256, 2, bytes.size() * 8, MPFR_RNDN); // 2^(số bit thực sự)
