...
namespace XapianLog
{
    ...
    void ComprehensiveLog::removeLogsUntilNearestEndCommand()
    {
        ...
        {
            if (it->command_type == XapianLog::CommandType::END)
            {
                // Tìm thấy END, xóa các log sau nó (không xóa END)
                xapian_doc_logs.erase(xapian_doc_logs.begin() + (xapian_doc_logs.size() - std::distance(xapian_doc_logs.rbegin(), it) - 1));
                break;
            }
        }
    }
