...
std::string getDbNameFromABI(const std::vector<uint8_t> &call_data)
{
    ...
    {
        throw std::runtime_error("Invalid call data: too short to contain dbname offset.");
    }
    ...
    {
        throw std::runtime_error(std::string("Failed to get dbname from ABI (out_of_range): ") + e.what());
    }
    catch (const std::exception &e)
    {
        throw std::runtime_error(std::string("Failed to get dbname from ABI: ") + e.what());
    }
}

DecodedQuerySearchArgs decodeQuerySearchCallData(const std::vector<uint8_t> &call_data)
{
    if (call_data.size() < 0 + 32 + 32)
    { // Selector + 2 offsets
        throw std::runtime_error("Invalid call data: too short for selector and offsets.");
    }
    ...
    uint64_t params_struct_start_offset = encoding::readUint256AsUint64(call_data, params_struct_offset_ptr);
    current_offset += 32;
    std::cerr << "params_struct_offset_ptr : " << params_struct_offset_ptr << std::endl;

    std::cerr << "params_struct_start_offset : " << params_struct_start_offset << std::endl;

    // Decode dbname (dùng offset đọc từ vị trí 4)
    result.dbname = encoding::readStringDynamic(call_data, 0);

    // Decode params (dùng offset đọc từ vị trí 36 và truyền offset bắt đầu của struct)
...
XapianSearcher::XapianSearcher(const std::string &db_path)
{
    ...
    {
        std::cerr << "Lỗi mở database '" << db_path << "': " << e.get_msg() << std::endl;
        throw;
    }
}
...
std::pair<std::vector<SearchResult>, Xapian::doccount> XapianSearcher::search(
    ...
    bool sort_ascending,
    const std::vector<RangeFilter> &range_filters, uint256_t blockNumber)
{
    ...
    try
    {
        ...
        if (stemmer_lang)
        {
            ...
            {
                std::cerr << "Cảnh báo: Không thể tạo stemmer/stopper: " << e.get_msg() << std::endl;
            }
        }
        ...
        for (const auto &q_str : queries)
        {
            if (!q_str.empty())
            {
                ...
                {
                    std::cerr << "Lỗi phân tích query '" << q_str << "': " << e.get_msg() << std::endl;
                }
            }
        }

        Xapian::Query keyword_query = parsed_queries.empty() ? Xapian::Query::MatchAll : (parsed_queries.size() == 1 ? parsed_queries[0] : Xapian::Query(combine_op, parsed_queries.begin(), parsed_queries.end()));

        Xapian::Query final_query = keyword_query;
        std::vector<Xapian::Query> value_range_queries;
        value_range_queries.reserve(range_filters.size() + 1); // thêm 1 cho slot 254

        // Các range filter bình thường
        for (const auto &filter : range_filters)
        {
            value_range_queries.emplace_back(
                Xapian::Query::OP_VALUE_RANGE, filter.slot,
                filter.start_serialised, filter.end_serialised);
        }

        // Tạo điều kiện lọc slot 254 >= blockNumber hoặc không tồn tại
        std::string block_serialised = Xapian::sortable_serialise(mvm::uint256_to_double(blockNumber));
        Xapian::Query slot254_ge_block = Xapian::Query(Xapian::Query::OP_VALUE_GE, 254, block_serialised);

        Xapian::Query has_slot254 = Xapian::Query(
        ...
        Xapian::Query slot254_filter = Xapian::Query(Xapian::Query::OP_OR, slot254_ge_block, slot254_is_null);

        value_range_queries.push_back(slot254_filter);

        Xapian::Query combined_range_query = (value_range_queries.size() == 1)
                                                 ? value_range_queries[0]
                                                 : Xapian::Query(Xapian::Query::OP_AND, value_range_queries.begin(), value_range_queries.end());

        final_query = Xapian::Query(Xapian::Query::OP_FILTER, final_query, combined_range_query);
        ...
        if (sort_by_value_slot)
        {
            try
            {
                enquire.set_sort_by_value_then_relevance(*sort_by_value_slot, sort_ascending);
            }
        ...
        for (Xapian::MSetIterator i = mset.begin(); i != mset.end(); ++i)
        {
            ...
            try
            {
                ...
                {
                    std::cerr << "Cảnh báo unserialise slot 253/254: " << e.get_msg() << std::endl;
                }
                ...
                std::cerr << "[DOCID: " << *i
                          ...
                          << ", Slot253: " << val253
                          << ", Slot254: " << val254
                          << ", Data: " << doc.get_data() << std::endl;

                results.push_back({*i, i.get_rank(), i.get_percent(), doc.get_data()});
            }
            catch (const Xapian::Error &e)
            {
                std::cerr << "Lỗi lấy document ID " << *i << ": " << e.get_msg() << std::endl;
            }
        }
    }
    catch (const Xapian::Error &e)
    {
        std::cerr << "Lỗi Xapian trong quá trình search: " << e.get_msg() << std::endl;
        throw;
    }
...
CppSearchParams decodeSearchParams(const std::vector<uint8_t> &encodedData)
{
    ...
    size_t queriesOffset = encoding::readUint256AsUint64(encodedData, currentOffset);
    currentOffset += 32;
    size_t queriesLength = encoding::readUint256AsUint64(encodedData, queriesOffset);
    params.queries = encoding::readStringFromData(encodedData, queriesOffset + 32, queriesLength);

    // 2. Giải mã 'prefixMap' (PrefixEntry[])
    size_t prefixMapOffset = encoding::readUint256AsUint64(encodedData, currentOffset);
    currentOffset += 32;
    size_t prefixMapLength = encoding::readUint256AsUint64(encodedData, prefixMapOffset);
    ...
    for (size_t i = 0; i < prefixMapLength; ++i)
    {
        ...
        // Giải mã 'key' (string)
        size_t keyOffset = encoding::readUint256AsUint64(encodedData, prefixMapDataOffset);
        size_t keyLength = encoding::readUint256AsUint64(encodedData, keyOffset);
        entry.key = encoding::readStringFromData(encodedData, keyOffset + 32, keyLength);
        prefixMapDataOffset += 32;

        // Giải mã 'value' (string)
        size_t valueOffset = encoding::readUint256AsUint64(encodedData, prefixMapDataOffset);
        size_t valueLength = encoding::readUint256AsUint64(encodedData, valueOffset);
        entry.value = encoding::readStringFromData(encodedData, valueOffset + 32, valueLength);
        prefixMapDataOffset += 32;
    ...
    size_t stemmerLangOffset = encoding::readUint256AsUint64(encodedData, currentOffset);
    currentOffset += 32;
    size_t stemmerLangLength = encoding::readUint256AsUint64(encodedData, stemmerLangOffset);
    params.stemmerLang = encoding::readStringFromData(encodedData, stemmerLangOffset + 32, stemmerLangLength);

    // 4. Giải mã 'stopWords' (string[])
    size_t stopWordsOffset = encoding::readUint256AsUint64(encodedData, currentOffset);
    currentOffset += 32;
    size_t stopWordsLength = encoding::readUint256AsUint64(encodedData, stopWordsOffset);
    size_t stopWordsDataOffset = stopWordsOffset + 32;

    for (size_t i = 0; i < stopWordsLength; ++i)
    {
        size_t wordOffset = encoding::readUint256AsUint64(encodedData, stopWordsDataOffset);
        size_t wordLength = encoding::readUint256AsUint64(encodedData, wordOffset);
        params.stopWords.push_back(encoding::readStringFromData(encodedData, wordOffset + 32, wordLength));
    ...
    params.sortByValueSlot = encoding::readUint256AsUint64(encodedData, currentOffset);
    currentOffset += 32;
    ...
    size_t rangeFiltersOffset = encoding::readUint256AsUint64(encodedData, currentOffset);
    currentOffset += 32;
    size_t rangeFiltersLength = encoding::readUint256AsUint64(encodedData, rangeFiltersOffset);
    ...
    for (size_t i = 0; i < rangeFiltersLength; ++i)
    {
        ...
        // Giải mã 'slot' (uint)
        filter.slot = encoding::readUint256AsUint64(encodedData, rangeFiltersDataOffset);
        rangeFiltersDataOffset += 32;

        // Giải mã 'startSerialised' (string)
        size_t startOffset = encoding::readUint256AsUint64(encodedData, rangeFiltersDataOffset);
        size_t startLength = encoding::readUint256AsUint64(encodedData, startOffset);
        filter.startSerialised = encoding::readStringFromData(encodedData, startOffset + 32, startLength);
        rangeFiltersDataOffset += 32;

        // Giải mã 'endSerialised' (string)
        size_t endOffset = encoding::readUint256AsUint64(encodedData, rangeFiltersDataOffset);
        size_t endLength = encoding::readUint256AsUint64(encodedData, endOffset);
        filter.endSerialised = encoding::readStringFromData(encodedData, endOffset + 32, endLength);
        rangeFiltersDataOffset += 32;
...
void XapianSearcher::dumpIndex()
{
    try
    {
        ...
        {
            std::cout << "Term: " << *it << ", Số lần xuất hiện: " << it.get_termfreq() << std::endl;
        }
    }
    catch (const Xapian::Error &e)
    {
...
std::pair<std::vector<SearchResult>, Xapian::doccount> XapianSearcher::searchABI(
    const std::vector<uint8_t> &abi_encoded_params)
{
    ...
    {
        throw std::runtime_error(std::string("searchABI: Failed to decode call data: ") + e.what());
    }
    ...
    std::optional<std::string> search_stemmer_lang = params.stemmerLang.empty()
                                                         ? std::nullopt
                                                         : std::make_optional(params.stemmerLang);

    std::optional<std::vector<std::string>> search_stop_words = params.stopWords.empty()
                                                                    ? std::nullopt
    ...
    std::optional<Xapian::valueno> search_sort_slot = (params.sortByValueSlot == CppSearchParams::NO_SORT_SLOT)
                                                          ? std::nullopt
                                                          : std::make_optional(static_cast<Xapian::valueno>(params.sortByValueSlot));
    ...
    {
        search_range_filters.push_back({static_cast<Xapian::valueno>(cpp_filter.slot),
                                        cpp_filter.startSerialised,
                                        cpp_filter.endSerialised});
    }

    // 3. Gọi hàm search gốc
    // *** Lưu ý: Tham số dbname từ decoded_args.dbname chưa được sử dụng ở đây ***
    // *** Hàm search gốc không nhận dbname. Cần điều chỉnh nếu logic XapianSearcher ***
    // *** phụ thuộc vào dbname (ví dụ: mở DB khác nhau dựa trên tên) ***
    std::cout << "Thông tin: Đang tìm kiếm trên DB (tên '" << decoded_args.dbname << "' từ ABI chưa được dùng trực tiếp trong hàm search này)" << std::endl;

    return this->search(
        search_queries, search_combine_op, search_default_op, search_prefix_map,
        search_stemmer_lang, search_stop_words, search_offset, search_limit,
        search_sort_slot, search_sort_ascending, search_range_filters);
}

std::vector<uint8_t> XapianSearcher::internalEncodeResultsArrayContent(const std::vector<SearchResult> &results)
{
    std::vector<uint8_t> encoded_content;
    // Ước tính kích thước có thể không chính xác hoàn toàn nhưng giúp tối ưu
    encoded_content.reserve(32 + results.size() * 180); // Điều chỉnh ước tính một chút
    ...
    // [độ dài mảng N] [offset đến elem 0] [offset đến elem 1] ... [offset đến elem N-1] [dữ liệu elem 0] [dữ liệu elem 1] ...
    // Offset được tính từ đầu của phần mã hóa mảng này (tức là từ sau độ dài N).
    ...
    // Tính toán kích thước của từng phần tử được mã hóa trước để xác định các offset
    std::vector<size_t> encoded_element_sizes(N);
    for (size_t i = 0; i < N; ++i)
    {
        // Kích thước phần cố định của struct SearchResult:
        // docid (32) + rank (32) + percent (32) + offset_to_data (32) = 128 bytes (0x80)
        size_t fixed_part_size = 4 * 32; // 128 bytes

        // Kích thước phần động (chuỗi string):
        // string_length (32) + padded_string_data
        size_t string_len = results[i].data.length();
        // Kích thước dữ liệu chuỗi được đệm thành bội số của 32
        size_t string_padded_len = ((string_len + 31) / 32) * 32;
        size_t dynamic_part_size = 32 + string_padded_len; // Kích thước slot độ dài + kích thước data đã đệm
    ...
    {
        // Offset của phần tử i = offset của phần tử (i-1) + kích thước mã hóa của phần tử (i-1)
 ...
 * @param results The vector of SearchResult objects for the current page (dynamic array part).
 * @return ABI encoded byte vector representing the SearchResultsPage tuple.
 */
std::vector<uint8_t> XapianSearcher::encodeSearchResultsPage(uint64_t total_count, const std::vector<SearchResult> &results)
{
    ...
    // encoding::appendUint256(encoded_page, 0x20); // <--- Dòng này sai logic ABI, cần xóa
    ...
    encoding::appendUint256(encoded_page, head_size); // offset = 64 bytes = 0x40
    ...
    std::vector<uint8_t> encoded_results = internalEncodeResultsArrayContent(results);

    // 4. Nối dữ liệu mảng đã mã hóa vào sau phần đầu
    encoded_page.insert(encoded_page.end(), encoded_results.begin(), encoded_results.end());
