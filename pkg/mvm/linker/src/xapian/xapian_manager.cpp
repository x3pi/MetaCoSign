...
tbb::concurrent_hash_map<std::string, std::shared_ptr<XapianManager>> XapianManager::instances;
...
std::string XapianManager::generateUuidLogicalId()
{
    ...
    uuid_unparse_lower(uuid_bin, uuid_str_buf.data()); // Chuyển sang chuỗi chữ thường
...
Xapian::Document XapianManager::clone_document(const Xapian::Document &source_doc)
{
    ...
    for (auto term = source_doc.termlist_begin(); term != source_doc.termlist_end(); ++term)
    {
...
std::shared_ptr<XapianManager> XapianManager::getInstance(const std::string &db_name, const mvm::Address &addr, bool isReset)
{
    ...
    tbb::concurrent_hash_map<std::string, std::shared_ptr<XapianManager>>::accessor accessor;

    // Kiểm tra xem instance đã tồn tại chưa (nếu không phải reset)
    if (!isReset && instances.find(accessor, db_path_str))
    {
        // Kiểm tra địa chỉ liên kết (nếu cần)
        if (accessor->second && accessor->second->address != addr)
        {
            // Địa chỉ không khớp, nhưng vẫn trả về instance hiện có theo logic gốc
        }
        // Cập nhật thời gian truy cập và trả về instance tìm thấy
        if (accessor->second)
            accessor->second->touch();
        return accessor->second;
    }

    // Nếu không tìm thấy hoặc là trường hợp reset, tạo mới instance
    if (instances.insert(accessor, db_path_str))
    {
        try
        {
            // Tạo instance mới và lưu vào map
            accessor->second = std::make_shared<XapianManager>(db_name, addr);
            if (accessor->second)
                accessor->second->touch(); // Cập nhật thời gian truy cập
            return accessor->second;
        }
        catch (const Xapian::Error &e)
        {
            instances.erase(accessor); // Xóa khỏi map nếu tạo lỗi
            throw;                     // Ném lại lỗi Xapian
        }
        catch (const std::exception &e)
        {
            instances.erase(accessor); // Xóa khỏi map nếu tạo lỗi
            throw;                     // Ném lại lỗi standard
        }
        catch (...)
        {
            instances.erase(accessor); // Xóa khỏi map nếu tạo lỗi
    ...
    {
        // Trường hợp insert thất bại: instance đã được tạo bởi thread khác
        // Kiểm tra địa chỉ liên kết (nếu cần)
        if (accessor->second && accessor->second->address != addr)
        {
            // Tạo đồng thời với địa chỉ khác
        }
        // Cập nhật thời gian truy cập và trả về instance tìm thấy
        if (accessor->second)
            accessor->second->touch();
...
XapianManager::XapianManager(const std::string &db_name, const mvm::Address &addr)
    : db(mvm::createFullPath(addr, db_name).string(), Xapian::DB_CREATE_OR_OPEN), // Mở hoặc tạo database
      address(addr),                                                              // Lưu địa chỉ liên kết
      last_access_time(std::chrono::steady_clock::now()),                         // Khởi tạo thời gian truy cập
      db_name(db_name)                                                            // Lưu tên database
{
}

// Lấy tên của database
std::string XapianManager::getDbName() const
{
    return this->db_name;
}
...
bool XapianManager::is_idle_for(std::chrono::minutes duration)
{
    ...
    auto idle_duration = std::chrono::duration_cast<std::chrono::minutes>(now - last_access_time);
...
Xapian::docid XapianManager::new_document(const std::string &data, uint256_t blockNumber)
{
    ...
    try
    {
        ...
        auto localId = generateUuidLogicalId(); // Tạo ID logic duy nhất
        Xapian::Document doc;
        ...
        // Thêm term chứa ID logic duy nhất (có tiền tố)
        ...
        // Ghi log thay đổi (staged)
        {
            ...
            std::lock_guard<std::mutex> lock(changes_mutex); // Khóa để thay đổi staged_changes_log
...
bool XapianManager::add_value(Xapian::docid did, Xapian::valueno slot, const std::string &value, bool isSerialise, uint256_t blockNumber)
{
    touch(); // Cập nhật thời gian truy cập
    try
    {
        ...
        if (isSerialise)
        {
            ...
            {
                return false; /* Định dạng số không hợp lệ */
            }
            catch (const std::out_of_range &)
            {
                return false; /* Số ngoài phạm vi */
            }
        }
        ...
        {
            // Tạo bản sao của document cũ
            Xapian::Document new_version_doc = clone_document(old_doc);
            // Thêm giá trị mới vào bản sao
            new_version_doc.add_value(slot, value_to_add);
            // Đặt block number tạo cho phiên bản mới này
            new_version_doc.add_value(253, blockNb_serialised);

            // Đánh dấu phiên bản cũ là bị thay thế tại block number hiện tại
            old_doc.add_value(254, blockNb_serialised);
            db.replace_document(did, old_doc); // Cập nhật phiên bản cũ
            db.add_document(new_version_doc);  // Thêm phiên bản mới (sẽ có docid mới)
        }
        // Ghi log thay đổi (staged)
        {
            ...
            logData.docid = did; // Log chỉ tham chiếu đến docid gốc
            logData.slot = slot;
            logData.value = value_to_log; // Log giá trị đã xử lý
...
bool XapianManager::add_term(Xapian::docid did, const std::string &term, uint256_t blockNumber)
{
    touch(); // Cập nhật thời gian truy cập
    try
    {
        ...
        else
        {
            ...
            db.replace_document(did, old_doc);
            db.add_document(new_version_doc); // Thêm phiên bản mới
        }
        // Ghi log thay đổi (staged)
        {
            std::lock_guard<std::mutex> lock(changes_mutex);
...
bool XapianManager::index_text(Xapian::docid did, const std::string &text_to_index, Xapian::termcount wdf_inc, const std::string prefix, uint256_t blockNumber)
{
    touch(); // Cập nhật thời gian truy cập
    try
    {
        ...
        if (existing_blockNb_serialised == blockNb_serialised)
        {
            term_generator.set_document(old_doc);                      // Liên kết với document cần cập nhật
            term_generator.index_text(text_to_index, wdf_inc, prefix); // Index text vào document đó
            db.replace_document(did, old_doc);                         // Lưu lại document đã cập nhật
        }
        else
        {
            Xapian::Document new_version_doc = clone_document(old_doc);
            term_generator.set_document(new_version_doc);              // Liên kết với bản sao
            term_generator.index_text(text_to_index, wdf_inc, prefix); // Index text vào bản sao
            new_version_doc.add_value(253, blockNb_serialised);        // Đặt block number tạo
...
bool XapianManager::set_data(Xapian::docid did, const std::string &new_data, uint256_t blockNumber)
{
    touch(); // Cập nhật thời gian truy cập
    try
    {
...
// Lấy giá trị từ một slot của document tại một block number, có tùy chọn unserialize
std::string XapianManager::get_value(Xapian::docid did, Xapian::valueno slot, bool isSerialise, uint256_t blockNumber)
{
    touch(); // Cập nhật thời gian truy cập
    try
    {
        ...
        return data; // Trả về dữ liệu gốc nếu không unserialize hoặc không có dữ liệu
    }
    catch (const Xapian::DocNotFoundError &)
    {
        return "";
...
std::vector<std::string> XapianManager::get_terms(Xapian::docid did, uint256_t blockNumber)
{
    touch(); // Cập nhật thời gian truy cập
    ...
    { /* Trả về vector rỗng (đã khởi tạo) */
    }
    catch (const Xapian::Error &)
    { /* Trả về vector rỗng */
    }
...
{
    touch();                                         // Cập nhật thời gian truy cập
    std::lock_guard<std::mutex> lock(changes_mutex); // Khóa để kiểm tra và xóa staged_changes_log
    ...
    {
        // Log lỗi ở đây nếu cần thiết, nhưng không output ra console nữa
...
{
    std::lock_guard<std::mutex> lock(changes_mutex); // Khóa để truy cập staged_changes_log
    if (comprehensive_log.xapian_doc_logs.empty())
    {
    ...
    for (const auto &entry : comprehensive_log.xapian_doc_logs)
    {
        try
        {
            std::vector<uint8_t> entry_bytes = entry.serialize(); // Serialize log entry
            combined_bytes.insert(combined_bytes.end(), entry_bytes.begin(), entry_bytes.end());
        }
        catch (const std::exception &)
        {
...
// Tính hash của các thay đổi liên quan đến tag (logic cụ thể không có trong snippet)
...
std::array<uint8_t, 32u> XapianManager::getComprehensiveStateHash()
{
    ...
    concatenated_data.insert(concatenated_data.end(), manager_log_hash.begin(), manager_log_hash.end());

    // Lấy hash của thay đổi tag (nếu có logic riêng)
    std::array<uint8_t, 32u> tags_hash = this->getCombinedTagsChangeHash();
    concatenated_data.insert(concatenated_data.end(), tags_hash.begin(), tags_hash.end());
...
namespace
{
    ...
    std::thread cleaner_thread([]
        ...
        while (cleaner_running.load()) // Chạy tant khi cờ là true
        {
            ...
            if (!cleaner_running.load()) break; // Kiểm tra lại cờ sau khi ngủ

            std::vector<std::string> keys_to_erase; // Danh sách key của instance cần xóa
            // Giai đoạn 1: Xác định các instance ứng viên để xóa (không giữ accessor lâu)
            for (auto it = XapianManager::instances.cbegin(); it != XapianManager::instances.cend(); ++it)
            {
    ...
    // Optional: Có thể tạo một đối tượng RAII để tự động gọi stopCleanerThread khi kết thúc scope toàn cục
    // struct CleanerStopper { ~CleanerStopper() { stopCleanerThread(); } } stopper_instance;
}
...
bool XapianManager::replay_log(const std::vector<XapianLog::LogEntry> &log_to_replay)
{
    ...
    std::lock_guard<std::mutex> db_lock(changes_mutex); // Khóa database trong suốt quá trình replay
    ...
    for (const auto &entry : log_to_replay)
    {
        ...
        try
        {
            ...
            success_op = std::visit(
                [this](const auto &data_arg) -> bool { // Capture 'this' để truy cập 'db'
                    ...
                    try
                    {
                        ...
                        if constexpr (std::is_same_v<T, XapianLog::NewDocData>)
                        {
                            ...
                            // Cần đảm bảo replay NEW_DOC xử lý đúng ID và các term/value mặc định nếu có
                            this->db.replace_document(data_arg.docid, doc); // Giả định replace hoạt động cho cả ID mới
                            return true;
                        }
                        else if constexpr (std::is_same_v<T, XapianLog::DelDocData>)
                        {
                            // Logic gốc dùng replace_document để soft delete. Replay nên làm tương tự.
                            // Nếu chỉ có docid trong log, cần fetch, thêm value 254 rồi replace.
                            // Hoặc nếu replay có nghĩa là xóa cứng:
                        ...
                        else if constexpr (std::is_same_v<T, XapianLog::AddValueData>)
                        {
                            ...
                            doc.add_value(data_arg.slot, data_arg.value); // Giá trị trong log đã được xử lý
                        ...
                        else if constexpr (std::is_same_v<T, XapianLog::IndexTextData>)
                        {
                            Xapian::Document doc = this->db.get_document(data_arg.docid);
                    ...
                    {
                        // Xử lý khi document không tìm thấy trong lúc replay (có thể bỏ qua)
    ...
    // Hàm này không tự commit, caller phải gọi commit_changes() nếu muốn lưu kết quả replay
    return true;
}

// Khôi phục trạng thái về lần commit cuối cùng bằng cách xóa log staged và mở lại DB
bool XapianManager::revertUncommittedChanges()
{
    ...
    try
    {
        ...
        // 2. Đóng và mở lại database để hủy các thay đổi chưa commit trong bộ nhớ Xapian
        db.close();                      // Đóng kết nối hiện tại
        db = Xapian::WritableDatabase(); // Gán bằng đối tượng rỗng để giải phóng tài nguyên cũ
        // Mở lại database từ đường dẫn đã lưu
        db = Xapian::WritableDatabase(mvm::createFullPath(address, db_name).string(), Xapian::DB_OPEN);
...
{
    tbb::concurrent_hash_map<std::string, std::shared_ptr<XapianManager>>::accessor acc;

    // Tìm instance trong map
    if (instances.find(acc, db_path_str))
    {
        std::shared_ptr<XapianManager> instance_ptr = acc->second; // Giữ một tham chiếu tạm thời
        try
        {
            ...
            if (instance_ptr)
            {
                ...
                // Bước 2: Dọn dẹp khác nếu cần (ví dụ: clear staged log)
                // std::lock_guard<std::mutex> lock(instance_ptr->changes_mutex);
                // instance_ptr->staged_changes_log.clear();
            }

            // Bước 3: Xóa khỏi map `instances`.
            // Việc này sẽ giảm use_count của shared_ptr. Nếu đây là tham chiếu cuối cùng
            // (cùng với instance_ptr), destructor sẽ được gọi khi instance_ptr ra khỏi scope.
            bool erased = instances.erase(acc); // accessor được giải phóng ở đây
            return erased;                      // Trả về true nếu xóa thành công
        }
        catch (const std::exception &)
        {
            // Lỗi xảy ra trong quá trình dọn dẹp *trước khi* xóa khỏi map.
            // Vẫn cố gắng xóa khỏi map để tránh entry bị treo.
            bool erased_after_error = false;
            tbb::concurrent_hash_map<std::string, std::shared_ptr<XapianManager>>::accessor error_acc;
            if (instances.find(error_acc, db_path_str))
            {
                erased_after_error = instances.erase(error_acc);
            }
            return erased_after_error; // Trả về kết quả của lần thử xóa sau lỗi
        }
        catch (...)
        {
            // Lỗi không xác định trong quá trình dọn dẹp.
            bool erased_after_error = false;
            tbb::concurrent_hash_map<std::string, std::shared_ptr<XapianManager>>::accessor error_acc;
            if (instances.find(error_acc, db_path_str))
            {
        ...
        // instance_ptr sẽ ra khỏi scope ở đây, có thể kích hoạt destructor nếu use_count về 0
    }
    return false; // Instance không được tìm thấy trong map
}
