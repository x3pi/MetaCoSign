...
namespace mvm
{
  ...
  AccountState MyGlobalState::get(const Address &addr, GasTracker *gas_tracker)
  {
    ...
    if (isCache && State::instanceExists(addr))
    {
      ...
      insert({MyAccount(addr, gBalance, gcode, gNonce), MyStorage(addr, blockContext.mvmId, isCache)});
      const auto acc = accounts.find(addr);
      if (gas_tracker != nullptr)
      {
    ...
    GlobalStateGet_return accountQueryData = GlobalStateGet(blockContext.mvmId, b_address + 12);
    ...
    if (accountQueryData.status == 1)
    {
      ...
      insert({MyAccount(addr, balance, code, nonce), MyStorage(addr, blockContext.mvmId, isCache)});
      const auto acc = accounts.find(addr);
      if (gas_tracker != nullptr)
      {
        gas_tracker->add_gas_used(getUnTouchedAddressGasCost());
  ...
  AccountState MyGlobalState::get(const Address &addr, GasTracker *gas_tracker, const uint256_t &nonce)
  {
    uint8_t b_address[32];
    mvm::to_big_endian(addr, b_address);
    const BlockContext &blockContext = get_block_context(); // Thêm dòng này
  ...
  AccountState MyGlobalState::create(
      const Address &addr, const uint256_t &balance, const Code &code, const uint256_t &nonce)
  {
    insert({MyAccount(addr, balance, code, nonce), MyStorage(addr, blockContext.mvmId, isCache)});
  ...
  void MyGlobalState::set_addresses_nonce_change(const Address &addr, const uint256_t &nonce)
  {
  ...
  const std::vector<std::vector<uint8_t>> MyGlobalState::get_storage_change()
  {
    ...
    if (this->isCache)
    {
      iterate_storage_changes([](const Address &addr, const uint256_t &key, const uint256_t &value)
                              {
        uint8_t  b_key[32];
  ...
  const std::vector<std::vector<uint8_t>> MyGlobalState::get_add_balance_change()
  {
    ...
    if (this->isCache)
    {
      iterate_add_balance_changes([](const mvm::Address &addr, const uint256_t &value)
                                  {
                                    auto state = State::getInstance(addr);
                                    auto balance = state->getBalance();
                                    state->setBalance(balance + value); });
    }
    return result;
  }
  ...
  const std::vector<std::vector<uint8_t>> MyGlobalState::get_sub_balance_change()
  {
    ...
    if (this->isCache)
    {
      iterate_sub_balance_changes([](const mvm::Address &addr, const uint256_t &value)
                                  {
                                    auto state = State::getInstance(addr);
                                    auto balance = state->getBalance();
                                    state->setBalance(balance - value); });
