...
func (s *MiningService) GetOrAssignJob(address common.Address, lastBlockNumber uint64) (*Job, error) {
	...
	if len(existingJobs) > 0 {
		fmt.Printf("Found existing '%s' job %s for address %s\n", JobStatusNew, existingJobs[0].JobID, address)
		return existingJobs[0], nil
	}

	// 2. Nếu không có job nào, tạo một job mới
	fmt.Printf("No '%s' job found for %s, creating a new one.\n", JobStatusNew, address)
	jobID := s.nextJobID
	s.nextJobID++
	...
	fmt.Printf("Created and assigned new job %s (Type: %s, Data: %s) to %s\n", newJob.JobID, newJob.JobType, newJob.Data, address)
	return newJob, nil
}
...
func (s *MiningService) CompleteJob(jobID string, assigneeAddress string) (*Job, error) {
	...
	fmt.Printf("Attempting to send %f reward from %s to %s for job %s\n", rewardAmount, s.rewardSenderAddress.Hex(), recipientAddr.Hex(), jobID)
	...
	if txErr != nil {
		...
		fmt.Printf("Warning: Failed to send reward transaction for job %s to %s: %v\n", jobID, recipientAddr.Hex(), txErr)
	...
	if err := s._indexJob(job); err != nil {
		...
		fmt.Printf("CRITICAL ERROR: Failed to update job %s after successful reward TX %s. Manual intervention may be required!\n", jobID, txHash.Hex())
	...
	if err := s._indexTransactionRecord(txRecord); err != nil {
		...
		fmt.Printf("Warning: Failed to index transaction record for job %s, tx %s: %v\n", jobID, txHash.Hex(), err)
	}

	fmt.Printf("Job %s completed by %s. Reward transaction: %s\n", job.JobID, assigneeAddress, txHash.Hex())
	return job, nil
}
...
func (s *MiningService) _searchAndParseJobs(queryStr string, limit int) ([]*Job, uint, error) {
	...
	for i := 0; i < mset.GetSize(); i++ {
		...
		if err := json.Unmarshal([]byte(doc.GetData()), &job); err != nil {
			fmt.Printf("Warning: could not unmarshal job JSON: %v\n", err)
			continue
		}
...
func (s *MiningService) _searchAndParseTransactionRecords(queryStr string, limit int) ([]*TransactionRecord, uint, error) {
	...
	for i := 0; i < mset.GetSize(); i++ {
		...
		if err := json.Unmarshal([]byte(doc.GetData()), &record); err != nil {
			fmt.Printf("Warning: could not unmarshal transaction record JSON: %v\n", err)
			continue
		}
