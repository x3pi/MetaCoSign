...
func (h *ValidatorHandler) HandleTransaction(ctx context.Context, chainState *blockchain.ChainState, tx types.Transaction, enableTrace bool) (types.Receipt, types.ExecuteSCResult, bool) {
...
func (h *ValidatorHandler) handleRegisterValidator(
	...
	if len(args) != 9 {
		logger.Error("số lượng tham số không đúng cho registerValidator")
		return nil, fmt.Errorf("số lượng tham số không đúng cho registerValidator")
	}
	...
	pub, _ := transaction.DerivePublicKeyFromEthTransaction(ethTx, chainState.GetConfig().ChainId)
	// Địa chỉ Ethereum từ public key
	compressedPubKey := crypto.CompressPubkey(pub)
	...
	pubKeyBls, err := chainState.GetAccountStateDB().GetPublicKeyBls(tx.FromAddress())
	if err != nil {
		return nil, fmt.Errorf("Lỗi khi lấy public key bls: %v", err)
	}
	pubKeyBlsHex := common.Bytes2Hex(pubKeyBls)
