...
func (h *ValidatorHandler) handleGetValidator(tx types.Transaction, chainState *blockchain.ChainState, method *abi.Method, inputData []byte) ([]byte, error) {
	if len(inputData) < 32 {
	...
	if validatorState == nil {
		...
		returnValues = []interface{}{
			...
			validatorState.CommissionRate(),             // commissionRate
