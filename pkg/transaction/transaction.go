...
func (t *Transaction) RHash() common.Hash {
	// Kiểm tra cache có giá trị hay chưa
	if cached := t.cachedRHash.Load(); cached != nil {
		return *cached // Trả về giá trị đã cache nếu có
	}
...
func (t *Transaction) ValidEthSign() bool {
	...
	logger.Error(ethTx)
	logger.Error("ethTx hash", ethTx.Hash())
	logger.Error("ethTx Type: ", ethTx.Type())
	...
	if err != nil {
		logger.Error(" ValidEthSign err", err)
		return false
	}
...
func DerivePublicKeyFromEthTransaction(tx *e_types.Transaction, chainID *big.Int) (*ecdsa.PublicKey, error) {
	if tx == nil {
		return nil, fmt.Errorf("transaction cannot be nil")
	}

	// Logic để xác định signer vẫn giữ nguyên như code của bạn
	...
	case e_types.AccessListTxType:
		if tx.ChainId() == nil || tx.ChainId().Sign() <= 0 {
			return nil, fmt.Errorf("EIP-2930 transaction is missing a valid chainID")
		}
		signer = e_types.NewLondonSigner(tx.ChainId())
	case e_types.DynamicFeeTxType:
		if tx.ChainId() == nil || tx.ChainId().Sign() <= 0 {
			return nil, fmt.Errorf("EIP-1559 transaction is missing a valid chainID")
		}
		signer = e_types.NewLondonSigner(tx.ChainId())
	...
	// Thay vì gọi e_types.Sender, chúng ta sẽ khôi phục public key thủ công.

	// 1. Lấy hash của giao dịch đã được ký.
	// Signer sẽ xử lý việc hashing cho các loại giao dịch khác nhau.
	txHash := signer.Hash(tx)

	// 2. Lấy các giá trị chữ ký (V, R, S).
	v, r, s := tx.RawSignatureValues()

	// 3. Định dạng chữ ký thành mảng byte 65-byte [R  S  V].
	// Hàm crypto.SigToPub yêu cầu V là recovery ID (0 hoặc 1).
	signature := make([]byte, crypto.SignatureLength)
	r.FillBytes(signature[0:32])
	s.FillBytes(signature[32:64])

	// Chuẩn hóa giá trị 'v' để lấy recovery ID.
	// Đối với các loại tx mới (sau EIP-155), giá trị v đã là 0 hoặc 1.
	// Đối với Legacy tx, chúng ta cần tính toán nó.
	recID := v
	if tx.Type() == e_types.LegacyTxType {
		if tx.ChainId().Sign() != 0 { // Giao dịch EIP-155
			// V = recID + chainID*2 + 35
			// => recID = V - chainID*2 - 35
			chainIDMul := new(big.Int).Mul(tx.ChainId(), big.NewInt(2))
			recID = new(big.Int).Sub(v, chainIDMul)
			recID.Sub(recID, big.NewInt(35))
		} else { // Giao dịch Homestead (trước EIP-155)
			// V = recID + 27
			// => recID = V - 27
			recID = new(big.Int).Sub(v, big.NewInt(27))
		}
	}
	signature[64] = byte(recID.Uint64())

	// 4. Khôi phục public key từ hash và chữ ký.
...
func DeriveSenderFromEthTransaction(tx *e_types.Transaction, chainID *big.Int) (common.Address, error) {
	...
	case e_types.LegacyTxType:
		...
		if chainID != nil && chainID.Sign() > 0 && tx.ChainId() != nil && tx.ChainId().Cmp(chainID) == 0 {
			logger.Info("signer 1")
			// If the tx has a EIP-155 ChainID that matches the current chain.
			signer = e_types.NewEIP155Signer(chainID)
			logger.Info("signer hash 1", signer.Hash(tx))
		} else if tx.ChainId() == nil || tx.ChainId().Sign() == 0 {
			// Pre-EIP155 transaction
			logger.Info("signer 2")

			signer = e_types.HomesteadSigner{}
			logger.Info("signer hash 2", signer.Hash(tx))
			...
			logger.Info("signer 3")

			signer = e_types.NewEIP155Signer(tx.ChainId())
			logger.Info("signer hash 3", signer.Hash(tx))
...
func (t *Transaction) ValidMaxFee(
	fromAccountState types.AccountState,
) bool {
	fee := t.MaxFee()
...
func (t *Transaction) ValidAmount(
	...
	value := big.NewInt(0)
	return t.ValidAmountSpend(fromAccountState, value)
}
...
// Chuyển transaction thành chuỗi JSON
func (t *Transaction) ToJSONString() string {
	b, err := t.MarshalJSON()
	if err != nil {
		return fmt.Sprintf("MarshalJSON error: %v", err)
	}
	return string(b)
}

// Hàm tiện ích cho types.Transaction
func TransactionToJSONString(tx types.Transaction) string {
	if tx == nil {
		return "nil"
	}
